...
int main(int argc,char** argv) {
   ...
   uint64_t n=atoi(argv[1]);
   uint64_t test_s=atoi(argv[2]);
   uint64_t exit_time = atoi(argv[3]);
   uint64_t* keys=new uint64_t[n];
   uint64_t* tkeys=new uint64_t[test_s];

   // Generate keys
   double start = gettime();
   for (uint64_t i=0;i<n;i++)
      keys[i]=(static_cast<uint64_t>(rand())<<32) | static_cast<uint64_t>(rand());
   for (uint64_t i=0;i<test_s;i++)
      tkeys[i]=keys[rand() % n];
   // Build tree
   Node* tree=NULL;
   for (uint64_t i=0;i<n;i++) {
   ...
   if(exit_time == 0) return 0;
      ...
      printf("lookup, %ld, %f\n",test_s,(test_s/1000000.0)/(gettime()-start));
   }
   else if(exit_time == 2){
      start = gettime();
      for (uint64_t i=0;i<test_s;i++) {
         uint8_t key[8];
         loadKey(tkeys[i],key);
         insert(tree,&tree,key,0,tkeys[i],8);
      }
      printf("insert, %ld, %f\n",test_s,(test_s/1000000.0)/(gettime()-start));
   }
   else {
